...
function DashboardPage() {
  ...
  const fetchClips = useCallback(
    async (pageNum, append = false) => {
      ...
      try {
        ...
        console.log(
          "✓ Clips fetched, thumbnails preloaded via HTTP/2 Server Push"
        );
      } catch (err) {
  ...
  const handleClipUpdate = useCallback((clipId) => {
    // Odśwież tylko ten konkretny klip
    setClips((prevClips) => {
      ...
      api
        ...
        .then((response) => {
          setClips((prev) => {
            const newClips = [...prev];
            // Zachowaj podstawowe dane, zaktualizuj nagrody
            newClips[index] = {
              ...newClips[index],
      ...
      return prevClips; // Return immediately, update happens async
    });
  }, []);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400 text-sm">
                {clips.length} {clips.length === 1 ? "klip" : "klipów"}{" "}
                załadowanych
                {hasSelection && (

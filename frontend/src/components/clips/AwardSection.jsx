...
function AwardSection({ clipId, initialAwards }) {
  ...
  const hasGivenAward = (awardName) => {
    return awards.some(
      (a) => a.award_name === awardName && a.user_id === user?.id
    );
  };
            ...
            myAwards.map((award) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    !alreadyGiven && handleGiveAward(award.award_name)
                  }
                ...
                >
                  ...
                  {loading && !alreadyGiven && (
                    <Loader className="animate-spin" size={16} />
                  )}
                  {alreadyGiven && (
                    <span className="text-green-500 text-sm">✓ Przyznano</span>
                  )}
                </button>
              );
            })
              ...
              >
                <div className="flex items-center gap-3 flex-1">
                  <span className="text-2xl">
                    {myAwards.find((a) => a.award_name === award.award_name)
                      ?.icon || "🏆"}
                  </span>
                  <div>
